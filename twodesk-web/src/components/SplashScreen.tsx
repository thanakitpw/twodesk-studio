...
// Bubble shape (speech bubble outline) — normal + mirrored tail
const BUBBLE_L = 'M5 5 Q5 0 12 0 L38 0 Q45 0 45 5 L45 22 Q45 27 38 27 L18 27 L10 35 L12 27 Q5 27 5 22 Z';
const BUBBLE_R = 'M45 5 Q45 0 38 0 L12 0 Q5 0 5 5 L5 22 Q5 27 12 27 L32 27 L40 35 L38 27 Q45 27 45 22 Z';

// Icons for each round — both bubbles show together, alternating who "speaks" (scale emphasis)
const BUBBLE_ICONS = [
  // Round 1: L speaks (บ้าน) | R idle (ตึก)
  {
    active: 'left' as const,
    left: 'M16 22 L25 12 L34 22 L34 27 L16 27 Z M22 27 L22 21 L28 21 L28 27', // บ้าน
    right: 'M18 27 L18 8 L32 8 L32 27 M22 12 L22 15 M26 12 L26 15 M22 18 L22 21 M26 18 L26 21', // ตึก
  },
  // Round 2: R speaks (ตลับเมตร) | L idle (บ้าน)
  {
...
function AlternatingBubbles() {
  ...
  return (
    <div
      className="absolute left-1/2 flex justify-between"
      style={{ width: '260px', top: 'calc(50% - 90px)', transform: 'translateX(-50%)' }}
    >
      {/* Left bubble (over left person) */}
      <motion.svg
        key={`left-${round}`}
        viewBox="0 0 50 40"
        className="h-[48px] w-auto"
        initial={{ scale: current.active === 'left' ? 0.7 : 0.9, opacity: 0, y: 6 }}
        animate={{
          scale: current.active === 'left' ? 1 : 0.85,
          opacity: current.active === 'left' ? 1 : 0.55,
          y: 0,
        }}
        transition={{ type: 'spring', stiffness: 160, damping: 14 }}
      >
        <path d={BUBBLE_L} fill="none" stroke="#c8c9cb" strokeWidth="1.5" strokeLinejoin="round" />
        <path d={current.left} fill="none" stroke="#c8c9cb" strokeWidth="1" strokeLinejoin="round" strokeLinecap="round" />
      </motion.svg>
      {/* Right bubble (over right person) */}
      <motion.svg
        key={`right-${round}`}
        viewBox="0 0 50 40"
        className="h-[48px] w-auto"
        initial={{ scale: current.active === 'right' ? 0.7 : 0.9, opacity: 0, y: 6 }}
        animate={{
          scale: current.active === 'right' ? 1 : 0.85,
          opacity: current.active === 'right' ? 1 : 0.55,
          y: 0,
        }}
        transition={{ type: 'spring', stiffness: 160, damping: 14, delay: 0.05 }}
      >
        <path d={BUBBLE_R} fill="none" stroke="#c8c9cb" strokeWidth="1.5" strokeLinejoin="round" />
        <path d={current.right} fill="none" stroke="#c8c9cb" strokeWidth="1" strokeLinejoin="round" strokeLinecap="round" />
      </motion.svg>
    </div>
  );
}
    ...
    <AnimatePresence>
      {phase < 10 && (
        ...
        >
          ...
          {/* ===== Scene 2: Crossfade TWODESK → Symbol ===== */}
          {phase === 2 && (
          ...
          {/* ===== Scene 3-6: Symbol stays → fade out ===== */}
          {phase >= 3 && phase <= 6 && (
            <motion.div
              className="absolute"
              initial={{ opacity: 1 }}
          ...
          {/* ===== Scene 3-6: People sitting on chairs beside symbol (table) ===== */}
          {phase >= 3 && phase <= 6 && (
            <motion.div
          ...
          {/* ===== Scene 4-5: Bubbles alternating L-R-L (with people still visible) ===== */}
          {(phase === 4 || phase === 5) && <AlternatingBubbles />}
        </motion.div>
      )}
    </AnimatePresence>
  );
